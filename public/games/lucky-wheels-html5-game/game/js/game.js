...
//wheel segments
var wheel_arr = [
	{src:'assets/item_wheel_01.png', highlight:'assets/item_wheel_01_h.png', color:'#FDCC09', regX:1, regY:223, point:10, type:0, percent:30,
		slot:{
				color:'#FDCC09',
...
var shareSettings = {
	...
	shareTitle:'Highscore on Lucky Wheels Game is [SCORE]PTS.',
	shareText:'[SCORE]PTS is mine new highscore on Lucky Wheels Game! Try it now!',
...
const playerData = {chance:0, score:0, point:0, bet:0};
const gameData = {spinning:false, stopped:true, rotateEaseNum:0, rotateInnerNum:0, wheelNum:0, wheelInnerNum:0, lightNum:16, ticketX:0, shape:'', touch:false, paused:true, spinDirection:true, spinSpeed:0, velocity:0, spindType:false, showStatus:false, fixedRotate:-1, fixedInnerRotate:-1};
...
const arrowData = {move:true, speed:0, rotatation:0, count:0};
...
function goPage(page){
	...
	switch(page){
		...
		case 'result':
			...
			resultScoreTxt.text = textStrings.resultScoreText.replace('[NUMBER]', addCommas(playerData.score));
			stopGame();
...
function startGame(){
	...
	if(gameSettings.gamePlayType){
		...
		playerData.score = playerData.point = gameSettings.gameBetPoint;
		playerData.bet = 0;
		betData.betNumber = 0;
	...
	statusTxt.text = textStrings.statusText_arr[0];
...
function toggleInstruction(con){
	if(con){
		if(gameSettings.gamePlayType){
			instructionTxt.text = textStrings.instructionTxt1;
		}else{
			instructionTxt.text = textStrings.instructionTxt2;
		}
...
function toggleBetNumber(con){
	...
	if(con == 'plus'){
		betData.betNumberPlus = gameSettings.gameCanBet;
	}else if(con == 'minus'){
		betData.betNumberPlus = -(gameSettings.gameCanBet);
	}else{
		betData.betNumberPlus = 0;	
...
function updateBetNumber(){
	...
	betData.betNumber = betData.betNumber >= gameSettings.gameMaxBet ? gameSettings.gameMaxBet : betData.betNumber;
...
function updateStat(){
	if(gameSettings.gamePlayType){
		chanceTxt.text = textStrings.chancesText.replace('[NUMBER]', playerData.chance);
		creditTxt.text = textStrings.creditText.replace('[NUMBER]', addCommas(Math.floor(playerData.point)));
	}else{
		creditTxt.text = textStrings.creditText.replace('[NUMBER]', addCommas(Math.floor(playerData.point)));
		betTxt.text = textStrings.creditText.replace('[NUMBER]', addCommas(Math.floor(playerData.bet)));	
	}
}
	...
	for(var n=0;n<wheel_arr.length;n++){
		...
		if(wheel_arr[n].slot != undefined){
			...
			if(wheel_arr[n].slot.color != ''){
				...
				newSlotColor.graphics.moveTo(0, 0).arc(0, 0, gameSettings.firstWheelRadius, 0, endAngle, false).closePath();
			...
			if(wheel_arr[n].slot.highlightColor != ''){
				...
				$.wheel['slotH'+n].graphics.moveTo(0, 0).arc(0, 0, gameSettings.firstWheelRadius, 0, endAngle, false).closePath();
		...
		for(var n=0;n<wheelSecond_arr.length;n++){
			...
			if(wheelSecond_arr[n].slot != undefined){
				...
				if(wheelSecond_arr[n].slot.color != ''){
					...
					thisWheel.graphics.moveTo(0, 0).arc(0, 0, gameSettings.secondWheelRadius, 0, endAngle, false).closePath();
				...
				if(wheelSecond_arr[n].slot.highlightColor != ''){
					...
					$.wheelInner['slotH'+n].graphics.moveTo(0, 0).arc(0, 0, gameSettings.secondWheelRadius, 0, endAngle, false).lineTo(0, 0).closePath();
...
function startSpinWheel(con){
	...
	statusTxt.text = textStrings.statusText_arr[1];
	...
	if(typeof memberData != 'undefined' && memberSettings.enableMembership){
		if(!gameData.physicsEngine){
			getUserResult("proceedSpin", playerData.bet);
		}else{
			var postData = {
				bet:playerData.bet,
				win:0,
			...
			updateUserPoint("proceedSpin", postData);
		}
	}else{
		proceedSpin();
	}
...
function startSpinWheelInner(){
	...
	var totalRound = Math.floor(gameData.spinSpeed/4);
	var totalRoundNum = 360 * totalRound;
	var toRotate = -(totalRoundNum + rotateNum);
	if(!gameData.spinDirection){
		toRotate = Math.abs(totalRoundNum + rotateNum);
	}
	totalRound = (gameData.physicsEngine == true && totalRound > 10) ? 10 : totalRound;
...
function checkWheelScore(){
	console.log(gameData.wheelNum, gameData.wheelInnerNum);
	...
	TweenMax.to(playerData, 1, {overwrite:true, onComplete:function(){
		...
		if(gameSettings.secondWheel){
			...
			if(wheelSegmentType == 1){
				...
				statusTxt.text = textStrings.statusText_arr[6];
				animateLights('lose');
				gameData.spinning = false;
				...
				statusTxt.text = textStrings.statusText_arr[2].replace('[NUMBER]', addCommas(wheelSegmentNumber));
				var speedTween = .5;
				TweenMax.to(itemStatusBg, speedTween, {overwrite:true, onComplete:function(){
					playSound('soundTone');
					statusTxt.text = textStrings.statusText_arr[2].replace('[NUMBER]', addCommas(wheelSegmentNumber)) +' x '+wheelInnerSegmentNumber;
					...
					TweenMax.to(itemStatusBg, speedTween, {overwrite:true, onComplete:function(){
						...
						statusTxt.text = textStrings.statusText_arr[3].replace('[NUMBER]', addCommas(winPoint));
						
						if(winPoint > 0){
							//win
							if(wheelSegmentType == 2){
								playSound('soundJackpot');
								statusTxt.text = textStrings.statusText_arr[5].replace('[NUMBER]', addCommas(winPoint));
							}else{
								playSound('soundWin');	
							}
							...
							playSound('soundLoss');
							//no win
							statusTxt.text = textStrings.statusText_arr[4];
							animateLights('lose');
							if(!gameSettings.gamePlayType){
								updateStat();
							}
...
function checkGameEnd(){
	...
	if(typeof memberData != 'undefined' && memberSettings.enableMembership){
		if(!gameData.physicsEngine){
			...
			matchUserResult("proceedCheckGameEnd", returnPoint);
		}else{
			var postData = {
				bet:playerData.bet,
			...
			updateUserPoint("proceedCheckGameEnd", postData);
		}
	}else{
		proceedCheckGameEnd();
	}
...
function getResultOnPercent(){
	...
	//console.log(gameData.percentageArray);
