...
var ballsContainer,ballsRevealContainer,ballsSelectContainer,cardContainer,tableContainer,starContainer;
var buttonStart,itemBall,itemBallBg,itemBallShadow,itemBallDim,itemBallGuess,itemBallBonus,itemBallHit,itemSphere,itemStick,itemShine,itemBar,itemBarBonus,buttonSphereStart,itemCard,itemBarUser,buttonLucky,itemNumberBg,itemNumberSelectBg,itemTable,selectTitleTxt,numberTitleTxt,buttonLeft,buttonRight;
$.prize = {};
...
function buildGameCanvas(){
	...
	numberTitleTxt.text = textStrings.numberTextDisplay;
	...
	for(var n=0; n<score_arr.length+extraBall;n++){
		...
		if(gameSettings.bonusBall){			
			if(n == 0){
				totalNum--;	
				textString = textStrings.matchTextDisplay.replace('[NUMBER]', totalNum);
				scoreString = textStrings.scoreTextDisplay.replace('[NUMBER]', addCommas(score_arr[totalNum-1].prize));
				scoreNum = score_arr[totalNum-1].prize;
			}else if(n == 1){
				textString = textStrings.bonusTextDisplay.replace('[NUMBER]', totalNum);
				scoreString = textStrings.scoreTextDisplay.replace('[NUMBER]', addCommas(bonusScore[0].prize));
