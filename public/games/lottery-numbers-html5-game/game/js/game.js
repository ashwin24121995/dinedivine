...
var textStrings = {
	selectTextDisplay:"Select [NUMBER] Numbers", //select number text display
	prizeTableDisplay:"Score Table", //score table text display
	numberTextDisplay:"Your Numbers", //your score text display
	matchTextDisplay:"MATCH [NUMBER]", //match text display
	scoreTextDisplay:"[NUMBER]PTS", //prize score text display
	bonusTextDisplay:"MATCH [NUMBER] + BONUS", //match bonus text display
	exitMessage:"ARE YOUR SURE\nYOU WANT TO QUIT THE GAME?", //go to main page message
	resultCompleteText:"YOU WON [NUMBER]PTS!", //result complete text display
...
var shareSettings = {
	...
	shareText:'[SCORE]PTS is mine new highscore on Lottery Numbers Game! Try it now!',
...
const gameData = {paused:true, sphereX:400, sphereY:325, cageRadius:225, radius:0, spin:false, selectNum:0, numberNum:0, numberArray:[], selectArray:[], winArray:[], buttonArray:[], indexArray:[], dimArray:[], matchNum:0, ballsArray:[], cageArray:[], revealArray:[], totalBall:[], ballNumber:[]};
const radiusTweenData = {radius:0};
const soundTweenData = {volume:0};
const optimizeData = {balls:40};
const cardData = {max:36, page:1, maxPage:0};
const ballData = {radius:60, scale:1};
	...
	switch(page){
		...
		case 'result':
			...
			if(gameData.matchNum != -1){
				...
				resultTitleTxt.text = textStrings.resultCompleteText.replace('[NUMBER]', addCommas(playerData.score));
...
function startGame(){
	...
	selectTitleTxt.text = textStrings.selectTextDisplay.replace('[NUMBER]', score_arr.length);
...
function proceedStartSpin(result){
	...
	selectTitleTxt.text = textStrings.prizeTableDisplay;
	...
	TweenMax.to(radiusTweenData, gameSettings.spinStartSpeed, {radius:gameSettings.spinSpeed, overwrite:true, onComplete:beginWinNumberTimer});
	TweenMax.to(soundTweenData, gameSettings.spinStartSpeed, {volume:1, overwrite:true, onUpdate:updateBallsVolume});
}
...
function endGame(){
	...
	TweenMax.to(soundTweenData, gameSettings.spinEndSpeed, {volume:.1, overwrite:true, onUpdate:updateBallsVolume});
