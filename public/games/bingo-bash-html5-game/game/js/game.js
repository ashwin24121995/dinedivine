 ...
 */
const gameSettings = {
	totalRevealBalls:35, //number of balls to reveal (0 - 75);
	revealTimer:4, //reveal number timer eg 5 secs
	totalPlayers:8, //total other players
	freeCenterBall:true, //enable free center ball
	highlightNumber:true, //toggle highlight number
	autoSelectNumber:false, //toggle auto select number
	enablePower:true, //toggle power for game
	powerData:{total:5, //total power badge appear in card
	...
	resultScore:{enable:false, score:500} //auto enable for member reward system score
};

const textStrings = {
	ballStatusText:"[NUMBER] BALLS LEFT", //balls left display text
	bingoStatusText:"[NUMBER] BINGO\"S LEFT", //bingo status display text
	exitMessage:"ARE YOUR SURE\nYOU WANT TO QUIT THE GAME?", //go to main page message
	resultCompleteText:"YOU WIN!", //result complete text display
	resultPlayerCompleteText:'[USER] WON!',  //result player complete text display
	resultFailText:"YOU DIDN\"T WIN!",  //result fail text display
	resultNoWinnerText:"NO WINNER!", //result when no winner
	resultScore:"YOU WONT [NUMBER]PTS",
}

//Social share, [SCORE] will replace with game score
const shareText = "SHARE THIS GAME"; //social share message
const shareSettings = {
	enable:true,
	options:['facebook','twitter','whatsapp','telegram','reddit','linkedin'],
	shareTitle:'Play Bingo Bash HTML5 Game.',
	shareText:"The excitement never ends in this Bingo Bash HTML5 Game! Try it now!",
 ...
 *
 */
const playerData = {complete:false};
const gameData = {paused:true, totalNum:75, bingo_arr:[], numbers_arr:[], numberIndex:0, balls_arr:[], reveal_arr:[], players_arr:[], historyBall_arr:[], playerBlink_arr:[], otherPlayersBlink_arr:[], stars_arr:[], complete:false, noWinner:false, firstReveal:true, power:0, powerTween:0, power_arr:[], powerType_arr:[]};
const patternData = {pattern_arr:[], index:0, count:0};
...
function checkStartGame(){
	...
	if(typeof memberData != 'undefined' && memberSettings.enableMembership){
		gameSettings.resultScore.enable = true;
		...
		if(memberData.ready){
			if(!checkMemberGameType()){
				...
				getUserResult("proceedGoGame", postData);
				//updateUserPoint("proceedGoGame");
			}
		}
	}else{
		goPage('game');
...
function proceedGoGame(result){
	if(result != undefined){
		gameData.numbers_arr = result.numbers;
	}
		...
		case 'result':
			...
			if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
				if(gameData.noWinner){
					resultTitleTxt.text = textStrings.resultNoWinnerText;
				}else{
					resultTitleTxt.text = textStrings.resultPlayerCompleteText.replace('[USER]', socketData.winners);
					if(playerData.complete){
						resultTitleTxt.text = textStrings.resultCompleteText;
						resultTitleTxt.text = textStrings.resultScore.replace('[NUMBER]', playerData.score);
					}
				}
			}else{
				if(gameData.noWinner){
					...
					if(playerData.complete){
						...
						resultTitleTxt.text = textStrings.resultFailText;
					}
				}
			}
...
function startGame(){
	...
	gameData.power = gameData.powerTween = gameSettings.powerData.value;
	...
	ballStatusTxt.text = ballStatusShadowTxt.text = textStrings.ballStatusText.replace('[NUMBER]', gameSettings.totalRevealBalls);
	updatePowerStatus();
	...
	if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
		if(socketData.host){
			...
			postSocketUpdate('revealnumbers', gameData.numbers_arr, true);
		}
	}else{
		if(typeof memberData != 'undefined' && memberSettings.enableMembership){
...
function createPlayerCard(){
	...
	for(var c=0; c<5; c++){
		...
		for(var r=0; r<5; r++){
			...
			if(c == 2 && r == 2){
				if(gameSettings.freeCenterBall){
	...
	if(gameSettings.enablePower){
		...
		for(var n=0; n<gameSettings.powerData.total; n++){
			if(gameSettings.freeCenterBall && $.player[allSlots_arr[n].c][allSlots_arr[n].r].isCenter){
	...
	if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
		postSocketUpdate('playercards', {index:socketData.gameIndex, numbers:playerData.storeNumbers});
	}
}
...
function createOtherPlayersCard(){
	...
	if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
		...
		for(var t=0; t<gameSettings.totalPlayers; t++){
			$.cards[t] = new createjs.Container();
			$.cards[t].x = containerX;
			$.cards[t].y = containerY;
...
function checkPlayerNumber(obj){
	if(gameData.reveal_arr.indexOf(obj.number) != -1 && !obj.checked){
		...
		if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
			...
			postSocketUpdate('updateusernumbers', {index:socketData.gameIndex, numbers:playerData.storeNumbers}, true);
		}
		...
		if(gameSettings.enablePower){
			if(obj.power){
				playSound('soundFill');
				updatePowerValue(gameSettings.powerData.powerValue);	
			}
		}
	}
}
...
function checkPlayerBingo(con){
	...
	bingoStatusTxt.text = bingoStatusShadowTxt.text = textStrings.bingoStatusText.replace('[NUMBER]', 5 - totalBingoLeft);
...
function startAnimateBingo(array, t){
	if(!gameData.complete){
		...
		if(t != undefined){
			...
			if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
				postSocketUpdate('bingo', socketData.players[socketData.gameIndex].username);
			}
			buttonBingoWin.visible = false;
...
function animateNewBall(){
	...
	for(var n=gameData.numberIndex; n<gameSettings.totalRevealBalls; n++){
...
function revealBall(){
	...
	ballStatusTxt.text = ballStatusShadowTxt.text = textStrings.ballStatusText.replace('[NUMBER]', gameSettings.totalRevealBalls - gameData.reveal_arr.length);
...
function revealBingo(num){
	...
	if(gameSettings.highlightNumber){
		highlightPlayerNumber();
	}
	if(gameSettings.autoSelectNumber){
...
function startRevealTimer(){
	var revealTimerNum = gameSettings.revealTimer;
	...
	if(!gameData.complete){
		if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
			if(socketData.host){
				...
				postSocketUpdate('revealballs','',true);
			}
		}else{
			revealBall();
			startRevealTimer();
...
function endGame(){
	TweenMax.to(gameContainer, 4, {overwrite:true, onComplete:function(){
		//memberpayment
		if(playerData.complete){		
			if(gameSettings.resultScore.enable){
				playerData.score = (gameSettings.totalRevealBalls - gameData.reveal_arr.length) / 100 * gameSettings.resultScore.score;
			}
		}
		...
		if(typeof memberData != 'undefined' && memberSettings.enableMembership){
			var postData = {
				type:"end",
				score:memberData.point+playerData.score,
				complete:playerData.complete,
				reveal:gameData.reveal_arr.length
			}
			getUserResult("proceedGoResult", postData);
...
function startPowerTimer(){
	TweenMax.to(powerContainer, gameSettings.powerData.updateTime, {overwrite:true, onComplete:powerTimeComplete});
}

function powerTimeComplete(){
	if(!gameData.complete){
		updatePowerValue(gameSettings.powerData.updateValue);
		updatePowerStatus();
		startPowerTimer();
	}
}
	...
	if(patternData.pattern_arr[patternData.index].total == patternData.count){
		...
		if(patternData.index < patternData.pattern_arr.length){
			...
			if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online) {
				if(socketData.host){
					dropBalls();
					postSocketUpdate('dropballs','',true);
				}
			}else{
				dropBalls();
			}
...
function shareLinks(action){
	...
	var shareTitle = shareSettings.shareTitle;
	var shareText = shareSettings.shareText;
